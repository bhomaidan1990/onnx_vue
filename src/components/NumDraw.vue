...
export default {
  ...
  methods: {
    async init() {
      ...
      const sessionOption = {
        executionProviders: [{ name: "webgl", deviceId: 0 }],
      };
      this.sess = await InferenceSession.create(
        "./models/mnist.onnx",
        sessionOption
      );
    ...
    async updatePredictions() {
      ...
      let input_tensor = new ort.Tensor("float32", img_arr, dims);

      const feeds = { a: input_tensor };
      const outputMap = await this.sess.run(feeds);
